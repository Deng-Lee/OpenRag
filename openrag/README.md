...
- 🔒 **企业级权限管控 (RBAC & ACL)**
  内置灵活的用户 (User) 与团队 (Team) 组织架构。支持细粒度的文件权限 (FilePermission) 授权模型，且提供全局操作审计日志 (AuditLog) 追踪，满足严苛的企业合规要求。
- ⚡ **高性能异步处理流水线**
...
2. **`User` & `Team`**: 系统的身份主体。用户可以加入多个工作空间，并在空间内被划分为不同团队进行协同。
3. **`File` (文件与目录)**: 虚拟文件节点。不仅记录基础元数据，还深度绑定了 RAG 属性（如 `processing_status`, `total_chunks`, `parser_type`）。
4. **`FilePermission` (文件权限)**: 采用多态设计，权限可以直接授予给单个 `User` 或整个 `Team`。
5. **`Task` (异步任务)**: 记录文档切片与 Embedding 任务的实时执行进度（`progress`）、状态及错误日志。
6. **`AuditLog` (审计日志)**: 详细记录系统内所有的“增删改查”及“分享”行为。
