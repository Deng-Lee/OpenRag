...
class ResolvedFileScope:
    ...
    def finite(cls, file_ids) -> "ResolvedFileScope":
        return cls(tuple(dict.fromkeys(int(file_id) for file_id in file_ids)))
    ...
    @property
    def allowed_file_ids(self) -> Optional[tuple[int, ...]]:
        return self.file_ids

    @property
    def is_verified_global(self) -> bool:
        return self.file_ids is None
...
class RetrievalService:
    ...
    def search(
        ...
        if resolved_file_scope.is_empty:
            return []
    ...
    def _effective_file_ids(
        ...
        """Intersect verified permissions with an optional requested file scope."""
        accessible = self._accessible_file_ids(user_id, workspace_id)
        if isinstance(accessible, ResolvedFileScope):
            accessible = accessible.as_list()
        if scope_file_ids is None:
            return accessible
        requested = {int(file_id) for file_id in scope_file_ids}
        if accessible is None:
            return sorted(requested)
        return [file_id for file_id in accessible if file_id in requested]
