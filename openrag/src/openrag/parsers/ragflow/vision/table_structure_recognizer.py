...
class TableStructureRecognizer(Recognizer):
    ...
    def __init__(self):
        try:
            ...
            logging.exception("Failed to initialize TableStructureRecognizer from local deepdoc models: %s", exc)
            disable_hf_fallback = os.environ.get("OPENRAG_DISABLE_HF_DOWNLOAD", "false").lower() in ("1", "true", "yes")
            if disable_hf_fallback:
                raise RuntimeError(
                    "TableStructureRecognizer local model init failed and OPENRAG_DISABLE_HF_DOWNLOAD is enabled"
                ) from exc
            super().__init__(
                self.labels,
                "tsr",
