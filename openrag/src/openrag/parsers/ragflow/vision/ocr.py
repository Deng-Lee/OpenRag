...
# Lightweight access to PARALLEL_DEVICES without importing the heavy
# common.settings module (which drags in elasticsearch, valkey, etc.).
class _SettingsProxy:
    """Minimal stand-in for common.settings used by vision code."""

    @property
    def PARALLEL_DEVICES(self) -> int:
        try:
            from common import settings as _real_settings
            return _real_settings.PARALLEL_DEVICES
        except Exception:
            import os
            return int(os.environ.get("PARALLEL_DEVICES", "0"))
    ...
    def __init__(self, model_dir=None):
        ...
        if not model_dir:
            try:
                ...
                logging.exception("Failed to initialize OCR from local deepdoc models: %s", exc)
                disable_hf_fallback = os.environ.get("OPENRAG_DISABLE_HF_DOWNLOAD", "false").lower() in ("1", "true", "yes")
