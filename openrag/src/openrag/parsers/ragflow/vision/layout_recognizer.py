...
class LayoutRecognizer(Recognizer):
    ...
    def __init__(self, domain):
        try:
            ...
            logging.exception("Failed to initialize LayoutRecognizer from local deepdoc models: %s", exc)
            disable_hf_fallback = os.environ.get("OPENRAG_DISABLE_HF_DOWNLOAD", "false").lower() in ("1", "true", "yes")
            if disable_hf_fallback:
                raise RuntimeError(
                    "LayoutRecognizer local model init failed and OPENRAG_DISABLE_HF_DOWNLOAD is enabled"
                ) from exc
        ...
        if os.environ.get("TENSORRT_DLA_SVR"):
            try:
                ...
                logging.warning(
                    "TENSORRT_DLA_SVR is set but dla_cli is not available; DLA layout client disabled."
                )
            else:
