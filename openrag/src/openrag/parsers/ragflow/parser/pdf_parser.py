...
from openrag.parsers.ragflow.vision import (
    OCR,
...
def _tag_contains_n(tag_result):
    """rag_tokenizer.tag() may return str ('n') or list (['n', ...]).
    Check if 'n' (noun) appears in the result."""
    if isinstance(tag_result, str):
        return tag_result.find("n") >= 0
    if isinstance(tag_result, (list, tuple)):
        return "n" in tag_result or any("n" in str(t) for t in tag_result)
    return False
...
class RAGFlowPdfParser:
    ...
    def _updown_concat_features(self, up, down):
        ...
        fea = [
            ...
            len(tks_down) == 1 and _tag_contains_n(rag_tokenizer.tag(tks_down[0])),
            len(tks_up) == 1 and _tag_contains_n(rag_tokenizer.tag(tks_up[0])),
        ]
        return fea
    ...
    def __ocr(self, pagenum, img, chars, ZM=3, device_id: int | None = None):
        ...
        for b in bxs:
            ...
            if total_count > 0 and self._is_garbled_by_font_encoding(
                box_chars, min_chars=5
            ):
                logging.info(
    ...
    def _extract_table_figure(
        ...
        # Page-number pattern: e.g. "— 15 —", "- 3 -", "15", "—15—"
        _PAGE_NUM_RE = re.compile(r'^[—\-–\s]*\d+[—\-–\s]*$')

        i = 0
        lst_lout_no = ""
        nomerge_lout_no_raw = []  # (lout_no, caption_page_number)
        # Pages where non-trivial content appears after a table group.
        # Used to block cross-page merging when body text sits between tables
        # (nomerge_lout_no_raw only catches captions/titles, not plain text).
        content_after_lout: dict[str, set[int]] = {}
        while i < len(self.boxes):
            ...
            if TableStructureRecognizer.is_caption(self.boxes[i]) or self.boxes[i][
                ...
                ltype = self.boxes[i].get("layout_type", "")
                txt = self.boxes[i].get("text", "").strip()
                # Skip page-footer noise that shouldn't block cross-page merging:
                #   - "reference" = footnotes
                #   - "title" matching page-number patterns like "— 15 —"
                is_page_footer = (
                    ltype == "reference"
                    or (ltype == "title" and _PAGE_NUM_RE.match(txt))
                )
                if not is_page_footer:
                    nomerge_lout_no_raw.append((lst_lout_no, self.boxes[i]["page_number"]))
            ...
            if need_image and self.boxes[i]["layout_type"] == "figure":
                ...
                continue
            if lst_lout_no:
                ltype_here = self.boxes[i].get("layout_type", "")
                txt_here = self.boxes[i].get("text", "").strip()
                is_noise = (
                    ltype_here == "reference"
                    or (ltype_here == "title" and _PAGE_NUM_RE.match(txt_here))
                    or not txt_here
                )
                if not is_noise:
                    content_after_lout.setdefault(lst_lout_no, set()).add(
                        self.boxes[i].get("page_number", 0)
                    )
            i += 1
        ...
        def table_x_bbox(bxs):
            """Bounding box over ALL boxes in a table group (not just the first)."""
            return {
        ...
        # Page-footer noise (footnotes, page numbers) was already filtered above.
        # nomerge_lout_no_raw stores (lout_no, caption_page) pairs.  A caption
        # on the SAME page as the source table should NOT block cross-page
        # merging (it belongs to the table, not separates two tables).  The
        # per-page filtering is applied in the merge loop below.
        nomerge_lout_no = set(lno for lno, _ in nomerge_lout_no_raw)
        nomerge_pages: dict[str, list[int]] = {}
        for lno, cpg in nomerge_lout_no_raw:
            nomerge_pages.setdefault(lno, []).append(cpg)
        logging.info(
            "[_extract_table_figure] tables_count=%s, nomerge=%s, content_after=%s",
            len(tables),
            {x for x in nomerge_lout_no if x},
            {k: sorted(v) for k, v in content_after_lout.items() if k},
        )

        # Log table groups for diagnosis
        tbls = sorted(
            [(k, bxs) for k, bxs in tables.items()],
            key=lambda x: (x[1][0]["page_number"], x[1][0]["top"], x[1][0]["x0"]),
        )
        for k, bxs in tbls:
            pg_set = set(b.get("page_number", 0) for b in bxs)
            bbox = table_x_bbox(bxs)
            logging.info(
                "[_extract_table_figure] table_group key=%s, pages=%s, box_count=%s, full_x=[%.1f,%.1f]",
                k, pg_set, len(bxs), bbox["x0"], bbox["x1"],
            )

        # Merge tables across pages (backward pass).
        # boxes already have global coordinates (page_cum_height baked in at
        # line ~970), so _y_dis works directly for cross-page comparison.
        tbls = sorted(
            [(k, bxs) for k, bxs in tables.items()],
        ...
        while i - 1 >= 0:
            ...
            src_max_pg = max(b["page_number"] for b in bxs0)

            if k0 in nomerge_lout_no:
                caption_pgs = nomerge_pages.get(k0, [])
                if all(cp <= src_max_pg for cp in caption_pgs):
                    logging.info(
                        "[_extract_table_figure] cross-page nomerge IGNORED: "
                        "%s -> %s (Captions on pg %s, all <= src pg %s)",
                        k0, k, caption_pgs, src_max_pg,
                    )
                else:
                    logging.info(
                        "[_extract_table_figure] cross-page SKIP: %s -> %s, "
                        "nomerge (caption pg %s > src pg %s)",
                        k0, k, caption_pgs, src_max_pg,
                    )
                    continue

            body_pgs = content_after_lout.get(k0, set())
            body_between = sorted(cp for cp in body_pgs if cp > src_max_pg)
            if body_between:
                logging.info(
                    "[_extract_table_figure] cross-page SKIP: %s -> %s, "
                    "body text between tables on pg %s (> src pg %s)",
                    k0, k, body_between, src_max_pg,
                )
                continue

            dst_min_pg = bxs[0]["page_number"]
            if dst_min_pg <= src_max_pg:
                continue
            pg_gap = dst_min_pg - src_max_pg
            if pg_gap > 3:
                logging.info(
                    "[_extract_table_figure] cross-page SKIP: %s -> %s, page gap=%s > 3",
                    k0, k, pg_gap,
                )
                continue

            mh = self.mean_height[dst_min_pg - 1]
            y_dis = self._y_dis(bxs0[-1], bxs[0])
            threshold = mh * 80
            logging.info(
                "[_extract_table_figure] cross-page CHECK: %s(src_end_pg=%s) -> %s(pg=%s), y_dis=%.1f, threshold=%.1f, mh=%.1f",
                k0, src_max_pg, k, dst_min_pg, y_dis, threshold, mh,
            )
            if y_dis > threshold:
                logging.info(
                    "[_extract_table_figure] cross-page SKIP: %s -> %s, y_dis=%.1f > threshold=%.1f",
                    k0, k, y_dis, threshold,
                )
                continue

            bbox0 = table_x_bbox(bxs0)
            bbox1 = table_x_bbox(bxs)
            if not x_overlapped(bbox0, bbox1):
                logging.info(
                    "[_extract_table_figure] cross-page SKIP: %s -> %s, no x_overlap (full_x=[%.1f,%.1f] vs [%.1f,%.1f])",
                    k0, k, bbox0["x0"], bbox0["x1"], bbox1["x0"], bbox1["x1"],
                )
                continue

            logging.info(
                "[_extract_table_figure] cross-page MERGE: %s(src_end_pg=%s) -> %s(pg=%s)",
                k0, src_max_pg, k, dst_min_pg,
            )
        ...
        logging.info(
            "[_extract_table_figure] cross_page_merges=%s, final_tables=%s",
            cross_page_merges,
            len(tables),
        )
            ...
            for (img, txt), poss in tbls_or_figs:
                ...
                for pn, left, right, top, bott in poss:
                    ...
                    if 0 <= local_pn < len(self.page_cum_height) - 1:
                        ...
                        logging.debug(
                            f"Skip out-of-range table/figure position pn={pn}, page_from={self.page_from}"
                        )
                if not local_poss:
                    logging.debug(
                        "No valid local positions for table/figure; skip insertion."
                    )
                    continue
                ...
                if bboxes:
                        ...
                        (
                            min_rectangle_distance(
                                (
                                    ...
                                    cand_left,
                                    cand_right,
                                    cand_top + self.page_cum_height[cand_pn],
                                    cand_bott + self.page_cum_height[cand_pn],
                                ),
                                rect,
                            ),
                            i,
                    ...
                    if dists:
                        ...
                        if (
                            self.boxes[insert_at]["bottom"]
                            < top + self.page_cum_height[pn]
                        ):
                            insert_at += 1
                else:
                    logging.debug(
                        "No text boxes available; append %s block directly.",
                        layout_type,
                    )
                self.boxes.insert(
                    insert_at,
    ...
    def crop(self, text, ZM=3, need_position=False):
        ...
        for pns, left, right, top, bottom in poss:
            if not pns:
                logging.warning(
                    "Empty page index list in crop; skipping this position."
                )
                continue
