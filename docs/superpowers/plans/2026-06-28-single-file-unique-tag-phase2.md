...
**测试运行约定（Phase 1 实测）:** worktree 无独立 venv；用主仓库 venv 跑、cwd 设在 worktree，`pytest.ini` 的 `pythonpath = . src` 会让 worktree 代码优先：
`& "e:\project\OpenRag\openrag\venv\Scripts\python.exe" -m pytest <文件> -v`（下文统一写作 `python -m pytest ...`）。新测试文件不在 `pytest.ini` 的 `python_files` 白名单内，**用显式路径调用即可收集**。跨文件复用 fixture 用 `from tests.test_service_api import _root, _stub_app_startup, client, db, owner, workspace, service_token_headers, service_token_write_headers`（Phase 1 验证可行）。
...
| `openrag/src/openrag/services/file_deletion.py`（改） | 新增 `_release_tag_and_soft_delete()`、`soft_delete_subtree()`、`physically_delete_soft_deleted_under_prefix()`；工具 `utcnow()` |
| `openrag/src/openrag/api/files_api.py`（改） | `DELETE /files/{id}` 异步软删除（文件单行 / 目录 subtree+`DELETE_PATH_PREFIX`）；`delete-path-prefix` 异步批量软删 + `deleted_before`；list/详情/content 读路径加 active 过滤 |
| `openrag/src/openrag/api/service_api.py`（改） | 新增 `DELETE .../documents/by-path`（异步软删 202 / 同步物删 200）；`GET by-tag`、`by-path`、replace 定位加 active 过滤 |
| `openrag/src/openrag/services/file_ingest.py`（改） | 父目录校验 / `ensure_directory_path` 只把 active 目录视为存在；同 uri active 查重 |
| `openrag/src/openrag/services/workspace_file_tree.py`（改） | 五个读函数加 `deleted_at IS NULL` |
| `openrag/src/openrag/retrieval/retrieval_service.py`（改） | `_accessible_file_ids()` 只返回 active；admin+workspace 无 active 返回空；chunk→file 回查丢弃软删 |
| `openrag/src/openrag/worker/task_worker.py`（改） | `DELETE_PATH_PREFIX` 物理清理改用按 `deleted_before` 的新 helper；`DELETE_FILE` 加 `deleted_at` 防御 |
| `openrag/tests/test_soft_delete_*.py`（建） | 各任务测试（见各 Task） |
...
Run: `python -m pytest tests/test_soft_delete_model.py -v`
...
Run: `python -m pytest tests/test_task_service_add_task.py -v`
...
Run（既有用例不回归）: `python -m pytest tests/test_service_api.py -q`
...
Run: `python -m pytest tests/test_soft_delete_helpers.py -v`
...
from openrag.models.task import Task as TaskModel
from openrag.services.task_service import TaskService
```
...
def _release_tag_and_soft_delete(db: Session, file: FileModel, *, user_id: int) -> TaskModel:
...
def physically_delete_soft_deleted_under_prefix(
    ...
    the task was enqueued are never touched.
    """
    p = prefix.rstrip("/")
    rows = (
        db.query(FileModel)
...
> `or_` 已在 `file_deletion.py` import（`delete_files_under_uri_prefix` 在用）。`FileModel`、`Workspace`、`delete_file_with_storage`、`delete_milvus_vectors_for_file`、`MinioStorage`、`HierarchyStorage` 均已在本模块。
...
Run: `python -m pytest tests/test_soft_delete_files_api.py -v`
Expected: FAIL — 当前 `background=True` 只建 `DELETE_FILE` 任务、不设 `deleted_at`/不释放 tag；目录不会入队 `DELETE_PATH_PREFIX`。

- [ ] **Step 3: 实现软删除分流**

In `openrag/src/openrag/api/files_api.py`，`delete_file` 的 `if background:` 分支（L792-811）替换为：
...
```python
        if background:
            ...
            if file.is_directory:
                prefix = file.uri.rstrip("/")
                watermark = soft_delete_subtree(db, file.workspace_id, prefix)
                task_service = TaskService(db)
                task_record = task_service.add_task(
                    workspace_id=file.workspace_id,
                    ...
                    payload={"path": prefix, "deleted_before": watermark.isoformat()},
                )
                db.commit()
...
> `background=false` 分支（`delete_file_with_storage`）保持不变：同步物删，行直接消失。
...
Run: `python -m pytest tests/test_soft_delete_files_api.py::test_delete_path_prefix_async_soft_deletes_and_writes_watermark -v`
...
```python
    if background:
        from openrag.services.file_deletion import soft_delete_subtree

        watermark = soft_delete_subtree(db, body.workspace_id, prefix)
        task_service = TaskService(db)
        task_record = task_service.add_task(
...
> 上方的 `n == 0 → 404/返回` 检查应只数 active 行——把 L869-876 的 count 查询加 `FileModel.deleted_at.is_(None)`，避免对全已软删的 prefix 反复入队。`background=false` 分支保持原同步物删。
...
Run: `python -m pytest tests/test_service_api_soft_delete.py -v`
...
Run: `python -m pytest tests/test_service_api_soft_delete.py tests/test_service_api.py -q`
Expected: 全 PASS。
...
def test_prefix_cleanup_skips_post_watermark_active_rows(session_factory):
    ...
    old = File(uri="/docs/old.txt", name="old.txt", owner_id=u.id, workspace_id=w.id,
               size=0, deleted_at=datetime.now(timezone.utc))
...
Run: `python -m pytest tests/test_worker_soft_delete_cleanup.py -v`
...
```python
        ...
        try:
            ...
            if deleted_before_raw:
                ...
                # legacy task without watermark: fall back to prefix-wide delete
                deleted_ids = delete_files_under_uri_prefix(db, workspace_id, path, workspace)
            return {
                "workspace_id": workspace_id,
                "path": path,
...
软删行必须对**所有读/替换/预览/父目录校验/检索候选**立即不可见。逐文件加过滤，每处配最小回归。

**Files:**
- Modify: `files_api.py`、`service_api.py`、`file_ingest.py`、`workspace_file_tree.py`
- Test: `openrag/tests/test_soft_delete_read_paths.py`
...
并 append 一个 service by-tag/by-path 的软删隐藏用例（复用 `tests.test_service_api` fixtures）：

```python
def test_service_by_tag_and_by_path_hide_soft_deleted():
    from tests.test_service_api import client as _c  # noqa: F401
    # 见下：实际放入 tests/test_service_api_soft_delete.py 更顺手
```

> 实施时把 service 层的两条断言写进 `tests/test_service_api_soft_delete.py`（已有 service fixtures）：软删行 `GET by-tag` → 404、`GET by-path` → 404。

- [ ] **Step 2: 跑测试确认失败**

Run: `python -m pytest tests/test_soft_delete_read_paths.py -v`
Expected: FAIL — 软删行仍出现在 list / detail / content。
...
- `list_files` 主查询（L504）：
  ...
  ```
- `_get_readable_file_or_404`（L587，覆盖 detail / content / preview / reprocess 入口）：
  ```python
  file = (
      db.query(FileModel)
  ...
  ```
- `delete_file` 的定位查询（L768）与 `move_file` 的源/目标定位：源行加 `FileModel.deleted_at.is_(None)`（软删行不可再删/移动）。
...
Run: `python -m pytest tests/test_soft_delete_read_paths.py tests/test_service_api_soft_delete.py tests/test_workspace_file_tree.py -v`
Expected: 全 PASS（含既有 tree 测试不回归）。

- [ ] **Step 5: 提交**

```bash
git add openrag/src/openrag/api/files_api.py openrag/src/openrag/api/service_api.py openrag/src/openrag/services/file_ingest.py openrag/src/openrag/services/workspace_file_tree.py openrag/tests/test_soft_delete_read_paths.py openrag/tests/test_service_api_soft_delete.py
git commit -m "feat(read-paths): filter deleted_at IS NULL across list/detail/tree/by-tag/by-path/parent-dir"
```

---

## Task 10: 检索读路径 — `_accessible_file_ids` active-only + 回查过滤
...
Run: `python -m pytest tests/test_soft_delete_retrieval.py -v`
Expected: FAIL — 软删文件 ID 仍被返回；admin 全删时返回 `None`（扩大为全量）。
...
> `_search_contextual`/`_search_flat` 已有 `if accessible is not None and len(accessible) == 0: return []`（L338），配合上面 active-only，软删文件不进候选。**回查兜底**（spec §6）：在 chunk→file 映射后若仍可能命中软删（allowlist 为 None=全量、或 >512 走 server 端过滤）的场景，加一道 `deleted_at` 回查——实施时定位 chunk 结果聚合处（`_max_score_per_file` 之后取 file 行的地方），对取出的 file 行 `if f.deleted_at is not None: continue`。若当前实现未单独取 file 行，则此兜底以 `_accessible_file_ids` active-only 为主、留注释说明 Phase 3 视检索改造再补。
...
git commit -m "feat(retrieval): _accessible_file_ids active-only; admin+workspace empty returns []"
```
...
Run:
```
python -m pytest tests/test_soft_delete_model.py tests/test_task_service_add_task.py tests/test_soft_delete_helpers.py tests/test_soft_delete_files_api.py tests/test_service_api_soft_delete.py tests/test_worker_soft_delete_cleanup.py tests/test_soft_delete_read_paths.py tests/test_soft_delete_retrieval.py tests/test_file_tag_model.py tests/test_file_ingest_tag.py tests/test_files_api_tag.py tests/test_service_api_tag.py tests/test_file_ingest_creates_dirs.py tests/test_service_api.py tests/test_workspace_file_tree.py -v
```
Expected: 全 PASS。
...
- 任务创建/commit 失败 → `deleted_at`/`tag` 一起回滚（旧文档仍可见、tag 未释放）。
- 前缀删除任务入队后，同 prefix 下新建的 active 行不被 worker 物理删除（worker 只删 `deleted_at <= deleted_before`）。
- 软删后**同 uri 裸上传仍 409**（spec §4.9 决策，uri 未释放）；`GET by-tag` 命中软删行 → 404。
...
- **Spec 覆盖**（§4.7/4.9/6/7、§8 Codex #1/#4/#8）：`deleted_at` 列+索引（T1/T2）、非提交 `add_task` + 单事务软删除（T3/T4）、三删除入口统一软删除（T5/T6/T7）、worker 按 `deleted_before` 水位清理（T8）、读路径全审计（T9）、检索 active-only + admin-empty 修复（T10）。✅ 覆盖。
...
- **占位符扫描**：每个实现步骤含完整代码/精确锚点，每个测试步骤含完整测试代码与可跑命令。唯二「视实现微调」处已显式标注：T8 worker 测试若 `__new__` 旁路不便则降级为 helper 直测；T10 回查兜底以 `_accessible_file_ids` active-only 为主。
- **类型/签名一致性**：`TaskService.add_task(...)` T3 定义、T4/T5/T6 复用；`_release_tag_and_soft_delete(db, file, *, user_id)`、`soft_delete_subtree(db, workspace_id, prefix) -> datetime`、`physically_delete_soft_deleted_under_prefix(db, workspace_id, prefix, deleted_before, workspace) -> list[int]` T4 定义、T5/T6/T7/T8 调用一致；`payload={"path", "deleted_before"}` T5/T6 写入、T8 读取一致。
- **承接 Phase 1**：迁移独立 `20260626_0005`（`down_revision=20260626_0004`）；`GET by-tag` 的 `deleted_at` 过滤在 T9 补齐（Phase 1 已标注遗留）。
- **测试约定**：沿用 Phase 1 实测的 venv/pythonpath/跨文件 fixture 复用/`_root()` 种子；新测试文件用显式路径收集。
